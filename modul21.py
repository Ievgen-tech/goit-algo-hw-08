# Завдання 2 (варіант AVL)
# Алгоритм пошуку суми всіх значень у AVL-дереві
...
class AVLTree:
    ...
    гарантуючи висоту O(log n) і часову складність O(log n) для вставки.
    """
    ...
    def _sum_subtree(self, node: AVLNode | None) -> int:
        """Рекурсивно обчислює суму значень піддерева з коренем у node.

        Складність: O(n) — потрібно відвідати кожен вузол.
        """
        if node is None:
            return 0
...
def sum_tree_values(tree: AVLTree) -> int:
    """Повертає суму всіх значень у переданому AVL-дереві."""
...
if __name__ == "__main__":
    ...
    print("\nСума всіх значень у дереві:", sum_tree_values(avl))  # очікується: 113
